...
class Inpatient(Patient):
    def __init__(self, name, age, patient_id, illness, room_number,
                 days_staying):
        super().__init__(name, age, patient_id, illness)
        self.room_number = room_number
        self.days_staying = days_staying

    # This overrides the Patient method
    def get_patient_info(self):
        return (
            f"{self.get_info()}, "
            f"Patient ID: {self.patient_id}, Illness: {self.illness}, "
            f"Room: {self.room_number}, Days Staying: {self.days_staying}"
        )

    # This is a new method for the child class
    def room_info(self):
        return (
            f"{self.name} is staying in room {self.room_number} "
            f"for {self.days_staying} days."
        )
...
def main():
    ...
    inpatient1 = Inpatient("Vik Danger", 31, "P102", "Cold", "204A", 3)
    ...
    print("Inpatient Test")
    print(inpatient1.get_patient_info())
    print(inpatient1.room_info())
    print()
